...
/* --------------------------------------------------------------------------
   Bagian 1: Pastikan kolom 'aktif' tersedia untuk soft-delete
   --------------------------------------------------------------------------
   - Soft delete dipakai agar data petugas tidak benar-benar hilang.
   - Jika kolom 'aktif' belum ada, otomatis ditambahkan.
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   Bagian 2: Soft delete data petugas
   --------------------------------------------------------------------------
   - Jika user klik hapus, maka update kolom 'aktif' jadi 0.
   - Tidak benar-benar DELETE supaya data tidak hilang permanen.
   -------------------------------------------------------------------------- */
if (isset($_GET['hapus'])) {
  ...
  if ($deleteResult) {
    ...
    // Jika gagal, coba tambahkan kolom aktif lagi lalu ulangi
    @mysqli_query($koneksi, "ALTER TABLE petugas ADD COLUMN aktif TINYINT(1) NOT NULL DEFAULT 1");
...
<!-- ------------------------------------------------------------------------
     Bagian 3: Header halaman & tombol tambah petugas
     ------------------------------------------------------------------------ -->
...
<?php
/* --------------------------------------------------------------------------
   Bagian 4: Ambil data dari database
   --------------------------------------------------------------------------
   - Jika mode edit, ambil data berdasarkan id_petugas.
   - Untuk daftar, hanya tampilkan petugas aktif (aktif=1).
   -------------------------------------------------------------------------- */
if (isset($_GET['edit'])) {
  $id = intval($_GET['edit']);
...
<!-- ------------------------------------------------------------------------
     Bagian 5: Notifikasi sukses/error
     ------------------------------------------------------------------------ -->
        ...
        <tr>
          ...
          <td>
            ...
            <a class="btn btn-sm btn-outline-primary" 
               href="/spp_app/index.php?page=petugas&edit=<?= $d['id_petugas'] ?>">
              <i class="bi bi-pencil"></i>
            </a>
            <a class="btn btn-sm btn-outline-danger" 
               href="/spp_app/index.php?page=petugas&hapus=<?= $d['id_petugas'] ?>" 
...
<?php 
/* --------------------------------------------------------------------------
   Bagian 7: Form edit (offcanvas)
   --------------------------------------------------------------------------
   - Jika user klik edit, tampilkan form edit di samping kanan layar.
   - User bisa update username, password (opsional), nama, dan level.
   -------------------------------------------------------------------------- */
if(isset($_GET['edit'])):
  $id=(int)$_GET['edit']; 
  $e=mysqli_fetch_assoc(mysqli_query($koneksi,"SELECT * FROM petugas WHERE id_petugas=$id"));
  ...
  if(isset($_POST['save'])){
    ...
    header("Location: /spp_app/index.php?page=petugas&msg=update");
    exit;
  }
?>
...
<div class="offcanvas offcanvas-end show" tabindex="-1" 
     style="visibility:visible; background:white; width:380px; border-left:1px solid #e5e7eb">
  ...
  <div class="offcanvas-body">
    <form method="post">
      <div class="mb-2">
        ...
        <input class="form-control" name="username" value="<?= htmlspecialchars($e['username']) ?>" required>
      </div>
      <div class="mb-2">
      ...
      <div class="mb-2">
        ...
        <input class="form-control" name="nama_petugas" value="<?= htmlspecialchars($e['nama_petugas']) ?>" required>
      </div>
      <div class="mb-3">
